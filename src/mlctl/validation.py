...
class ProjectValidator:
    ...
    def _validate_instance_type(self, ml_config: Dict[str, Any]):
        ...
        if self.config.is_instance_type_allowed(instance_type, team):
            console.print(f"[green]✓[/green] Instance type '{instance_type}' is allowed")
        else:
            self.errors.append(
                f"Instance type '{instance_type}' is not in the allowlist. "
                f"Allowed types: {self.config.org_config.get('allowed_instance_types', [])}"
            )
            console.print(f"[red]✗[/red] Instance type '{instance_type}' is not allowed")
    ...
    def _validate_scripts(self):
        ...
        for script in optional_scripts:
            ...
            if script_path.exists():
                if self._check_python_syntax(script_path):
                    console.print(f"[green]✓[/green] Optional script '{script}' syntax is valid")
                else:
                    self.warnings.append(f"Optional script '{script}' has syntax errors")
                    console.print(f"[yellow]![/yellow] Optional script '{script}' has syntax errors")
    ...
    def _print_results(self):
        ...
        if self.errors:
            console.print(f"[red bold]Validation failed with {len(self.errors)} error(s)[/red bold]")
