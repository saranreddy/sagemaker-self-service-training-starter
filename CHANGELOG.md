...
- **PyTorch container dependency pins**: Fixed PyTorch requirements for Python <3.11 to include `numpy>=1.24.1,<2` and `scikit-learn>=1.2.1,<1.6` (prevents numpy 2.x resolution and missing sklearn errors). These constraints match the PyTorch DLC's installed versions, so nothing is downgraded in the container. Dropped torchvision entirely (unused by example/template). PyTorch CI job now actually imports and tests modules instead of only doing a dry-run.
- **Doctor quota lookup**: Fixed `scripts/doctor.sh` to look up SageMaker quotas by exact name via `list-service-quotas` instead of using hardcoded quota codes. Properly handles `None` quota values as unknown.
