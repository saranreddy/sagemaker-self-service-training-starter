...
### Fixed
- **Model package group creation blocker**: Fixed `ensure_model_package_group` to handle AWS SageMaker's actual `ValidationException` error code (with message containing "does not exist") instead of only `ResourceNotFound`. The group is now created with deployment tags so pre-destroy cleanup can find and delete it. Existing untagged groups are now tagged with `add_tags` to ensure pre-destroy finds them (verified with unit tests; not yet exercised against a live account). Added unit tests with botocore Stubber covering exists, ValidationException does-not-exist → create with tags, and other ClientError → raises.
- **Version number consistency**: `mlctl --version` now correctly shows 0.1.2. Version is read from `__init__.py` in both CLI and setup.py (single source of truth).
...
- **Pipeline failure visibility**: `mlctl status` now displays each failed step's `FailureReason` after the step table (with step name) for better debugging. Failure reasons are properly escaped to avoid Rich markup parsing crashes on reasons containing `[...]` text.
- **Team tag reading**: `mlctl list` now reads the `Team` value from pipeline tags instead of showing hardcoded "unknown".
- **Smoke test failure reporting**: `scripts/smoke-test.sh` now prints failed step `FailureReason` when pipeline fails, detects account-level service limit errors (ResourceLimitExceeded for training/processing quotas at 0 Instances), and prints actionable guidance to request quota increases via AWS Service Quotas console. Properly distinguishes training vs processing quota errors. Instance type is now configurable via `SMOKE_INSTANCE_TYPE` environment variable (defaults to ml.m5.large), which is automatically added to the allowed instance types list.
- **Project name validation**: Terraform `var.project_name` validation regex now caps at 40 characters to ensure the generated bucket name (`${project_name}-artifacts-${account_id}`) stays under S3's 63-character limit. Updated error message to reflect 3-40 character range.
...
- **PyTorch container dependency pins**: Fixed PyTorch requirements for Python <3.11 to include `numpy>=1.24.1,<2` and `scikit-learn>=1.2.1,<1.6` (prevents numpy 2.x resolution and missing sklearn errors). These constraints match the PyTorch DLC's installed versions, so nothing is downgraded in the container. Dropped torchvision entirely (unused by example/template). The PyTorch CI job now installs the example requirements (`--only-binary=:all:`), checks torch↔numpy interop with `torch.from_numpy`, and imports `train`/`evaluate`.
- **Doctor quota lookup**: Fixed `scripts/doctor.sh` to look up SageMaker quotas by exact quota name via `list-service-quotas` (JSON output, so the query runs once across all result pages) instead of hardcoded quota codes; missing/`null` values are treated as unknown and never fail doctor.
- **AWS CLI version documentation**: Updated README to document that both AWS CLI v1 and v2 are supported.

### Added
- **SageMaker quota check in doctor**: `scripts/doctor.sh` now checks SageMaker service quotas for ml.m5.large training and processing instance types by looking up quotas by exact name. If quotas are 0, prints a warning (not a failure) with guidance to request increases. Degrades gracefully if service-quotas access is denied or AWS CLI too old.
- **Container pin validation improvements**: Rewrote `tests/test_container_pins.py` to use `packaging.requirements.Requirement` and `packaging.markers` for proper environment marker evaluation across Python 3.8, 3.9, 3.10. Now rejects unmarked framework lines (sklearn/xgboost/torch) and validates that container pins match container Python versions. Accepts both single-quote and double-quote marker forms. For PyTorch with torch 2.1.0, requires numpy<2 constraint to avoid runtime errors. Updated `CONTAINER_VERSIONS` table to only validate framework packages. Added source links to container repos and DLC releases. Test now covers templates in addition to examples. Added `packaging` to requirements-dev.txt.
- **Documentation enhancements**: Added explanation of `python_version` marker split in README (container vs local pins), documented `brew install libomp` requirement for local xgboost on macOS, documented `SMOKE_INSTANCE_TYPE` environment variable and SageMaker quota prerequisites for smoke test, and updated CI job list to mention Python 3.13 and the PyTorch job, which installs the example requirements and imports torch/numpy/sklearn and the train/evaluate modules (it does not run training).
